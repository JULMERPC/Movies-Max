...
import com.puma.videomax.data.local.db.QueueWindowSql
...
import com.puma.videomax.data.local.mediastore.MediaStoreVideoScanner
...
import com.puma.videomax.domain.repository.QueueWindow
...
class VideoRepositoryImpl @Inject constructor(
	...
	private fun collectNomediaDirs(): Set<String> {
		...
		val roots = listOfNotNull(
			runCatching { android.os.Environment.getExternalStoragePublicDirectory(null) }.getOrNull(),
			runCatching { android.os.Environment.getExternalStorageDirectory() }.getOrNull()
		)
		for (root in roots) {
			...
			try {
				...
				// skip if we can't walk the tree
			}
		}
		return dirs
	}
